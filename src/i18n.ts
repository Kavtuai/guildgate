...
const messages: Record<SupportedLocale, Record<string, string>> = {
  en: {
    ...
    IDEMPOTENCY_RESERVATION_LOST: "The operation reservation changed before completion. Retry with the same idempotency key.",
    ...
    LOCK_OWNERSHIP_LOST: "The distributed lock expired before the operation completed.",
    OWNER_REQUIRED: "Only a configured bot owner can perform this operation.",
    RATE_POLICY_RESET_DENIED: "This rate policy cannot be reset from the owner API.",
    UPSTREAM_TIMEOUT: "A required service did not respond in time.",
    REQUEST_ABORTED: "The request was cancelled before the operation finished.",
  ...
  tr: {
    ...
    IDEMPOTENCY_RESERVATION_LOST: "İşlem tamamlanmadan önce rezervasyon değişti. Aynı idempotency anahtarıyla yeniden deneyin.",
    ...
    LOCK_OWNERSHIP_LOST: "Dağıtık kilidin süresi işlem tamamlanmadan doldu.",
    OWNER_REQUIRED: "Bu işlemi yalnızca yapılandırılmış bot sahibi yapabilir.",
    RATE_POLICY_RESET_DENIED: "Bu rate politikası bot sahibi API'sinden sıfırlanamaz.",
    UPSTREAM_TIMEOUT: "Gerekli servis zamanında yanıt vermedi.",
    REQUEST_ABORTED: "İstek işlem tamamlanmadan iptal edildi.",
