...
npm init -y
npm install /path/to/kavtuai-guildgate-1.1.1.tgz
...
The release tag must equal `v` plus the `package.json` version. The package version must pass `npm run release:verify`; the release tag must match it exactly. Maintainer review material stays in the source repository and is not included in the npm tarball.
